...
from helpers.function_helpers import load_data, unit_scaling, norm_scaling, eval_dataloaders
from helpers.plotting_helpers import plot_parity
import argparse

parser = argparse.ArgumentParser(description="SVMOGP Process-Structure Linkage")
...
parser.add_argument("--n_inducing", default=0.1, type=float, help="% inducing points of training set size")
...
fname_model = 'mogp_model_state_psNGsm_test.pth'
fname_lik = 'mogp_likelihood_state_psNGsm_test.pth'

fname = './data/data.h5'
data = load_data(fname, test_size=0.2, random_state=10, device='cpu')

train_x = data['input_train'].float()
test_x = data['input_test'].float()
train_y = data['mid_train'][:,:args.n_pcs].float()
test_y = data['mid_test'][:,:args.n_pcs].float()
...
train_y = mid_scaler.inverse_transform(train_y.numpy())
test_y = mid_scaler.inverse_transform(test_y.numpy())
